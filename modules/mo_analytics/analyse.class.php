...
class analyse{
  ...
  function addSymptom($value) {
    ...
    if( !in_array($value, $this->symptoms) ){
      if($data = $db->select_row('item_name','ICD10-items','item_nr="'.$value.'"') ){
        $this->symptoms[] = array('code'=>$value,'name'=>$data['item_name']);
      }
    }
  }
