...
<?php
  ...
  if( isset($_SESSION['analyse']) && ( $analyse_id = $_SESSION['analyse']->_get( 'analyse_id' ) ) != '' ){
    echo '<a href="index.php?module=analyse&action=overview&analyse_id='.$analyse_id.'">{L:analyse:overview}</a>',"\n";
  }
  ...
  if( !isset( $_POST['meds'] ) && !isset( $_POST['symptoms'] ) && !isset( $_POST['f_additional'] ) && !isset( $_GET['action'] ) ){
    ...
    // if is set, then call dropData function from object
    // all data are removed and a new id will be generated
    if( isset($_GET['action']) ) {

      // delete all data and go to start page
      if( $_GET['action']=='drop_data' ) {
      ...
      } elseif( $_GET['action']=='process_data' ) { // send data to extern server
    ...
    $module_markers['###BUTTON_RESET###'] =  '<a href="./index.php?module=analyse&action=drop_data" class="button delete" titel="Alle Daten werden gelöscht!!" '
        .'onclick=" return confirm(\'Wollen Sie die Daten wirklich löschen?\'); return false;">{L:system:drop_data} </a>';
