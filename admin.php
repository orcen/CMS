...
if( isset( $_POST['f_login'] ) ) {
  if( $result = $user->login($_POST['f_username'],$_POST['f_password']) ) {
...
if( isset($_GET['logout']) && filter_var($_GET['logout'],FILTER_VALIDATE_BOOLEAN) ){
  $user->logout();
  header('location: '. $_sC->_get('domain').'admin.php');
}




if( false == $user->isLoaded() ) {
  $form = new form();
...
$markers = array(
 ...
 "###TOPNAVIGATION###" =>  !$user->isLoaded() ? '' :'<ul><li><a href="?mod=sys_import">{L:system:import}</a></li><li><a href="?mod=sys_overview">{L:system:analyse_overview}</a></li><li><a href="?mod=sys_datasheets">{L:system:datasheet}</a></li></ul>' ,
